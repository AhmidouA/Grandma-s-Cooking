...
const userController = {
  ...
  async sendPasswordResetEmail(req, res) {
    ...
    try {
      ...
      console.error(chalk.bgRedBright(err));
      console.error(
        chalk.bgRedBright(`l'utilisateur ${username} n'a pas été trouvé`)
      );
      res.redirect("/signup");
    }
  },
  ...
  async resetPasswordIndexPage(req, res) {
    ...
    try {
      ...
      console.log(
        chalk.bgBlue("{ resetPasswordToken>>>>>>> }", reset.resetPasswordToken)
      );
      console.log(
  ...
  async resetPassword(req, res) {
    ...
    try {
      ...
      if (password !== password2) {
        console.log(
          chalk.red(`Le nouveaux mots de passe ne correspondent pas`)
        );
      ...
      const resetToken = Reset.findOneAndUpdate(
        { resetPasswordToken: token },
        updateReset
      );
      if (!resetToken) {
